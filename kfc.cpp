...
//---------------------------------------------------------------------------
KImageGray KImageGray::Opening(const int& nType, const int& kernelSize, const int& neighborType)
{
    // First perform erosion and then dilation
    KImageGray erodedImage = this->BinaryErode(nType, kernelSize, neighborType);
    return erodedImage.BinaryDilate(nType, kernelSize, neighborType);
}

KImageGray KImageGray::Closing(const int& nType, const int& kernelSize, const int& neighborType)
{
    // First perform dilation and then erosion
    KImageGray dilatedImage = this->BinaryDilate(nType, kernelSize, neighborType);
    return dilatedImage.BinaryErode(nType, kernelSize, neighborType);
}

KImageGray KImageGray::SubtractImages(const KImageGray& img1, const KImageGray& img2) {
    KImageGray result(img1.Row(), img1.Col());
    for (int i = 0; i < img1.Row(); i++) {
        for (int j = 0; j < img1.Col(); j++) {
            int diff = static_cast<int>(img1[i][j]) - static_cast<int>(img2[i][j]);
            result[i][j] = static_cast<unsigned char>(std::max(0, std::min(255, diff)));
        }
    }
    return result;
}
...
KImageGray KImageGray::BinaryErode(const int& nType, const int& kernelSize, const int& neighborType)
{
    KImageGray igOut = *this;
    int start = kernelSize / 2;
    int end = kernelSize - start;

    for(int i = start; i < Row() - end; i++)
    {
        for(int j = start; j < Col() - end; j++)
        {
            if(nType == _FOREGROUND && _ppA[i][j])
            {
                for(int m = -start; m <= end; m++)
                {
                    for(int n = -start; n <= end; n++)
                    {
                        if((neighborType == 8 || (m == 0 || n == 0)) && _ppA[i + m][j + n] == 0)
                        {
                            igOut._ppA[i][j] = 0;
                            m = end + 1; // 루프 종료
                            break;
            ...
            else if(nType == _BACKGROUND && _ppA[i][j] == 0)
            {
                for(int m = -start; m <= end; m++)
                {
                    for(int n = -start; n <= end; n++)
                    {
                        if((neighborType == 8 || (m == 0 || n == 0)) && _ppA[i + m][j + n] == 255)
                        {
                            igOut._ppA[i][j] = 255;
                            m = end + 1; // 루프 종료
                            break;
...
KImageGray KImageGray::BinaryDilate(const int& nType, const int& kernelSize, const int& neighborType)
{
    KImageGray igOut = *this;
    int start = kernelSize / 2;
    int end = kernelSize - start;
    ...
    for(int i = start; i < Row() - end; i++)
    {
        for(int j = start; j < Col() - end; j++)
        {
            if(nType == _FOREGROUND && _ppA[i][j] == 0)
            {
                for(int m = -start; m <= end; m++)
                {
                    for(int n = -start; n <= end; n++)
            ...
            else if(nType == _BACKGROUND && _ppA[i][j] == 255)
            {
                for(int m = -start; m <= end; m++)
                {
                    for(int n = -start; n <= end; n++)
